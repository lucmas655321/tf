...
show_path=True adds the full path next to each label: handy to copy it
into tf_getBlockContent without reconstructing it manually.
...
Workflow tip: tf_tree → drill into block → tf_getBlockContent for code.
